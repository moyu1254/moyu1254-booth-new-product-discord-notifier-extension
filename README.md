...
設定画面の `今すぐ実行` から手動チェックもできます。実行結果は `最終実行結果` に保存され、ポップアップ側には未読件数と直近の商品一覧が反映されます。
...
Discord 通知を無効にしても、拡張機能内の `新着商品一覧` と未読バッジは更新されます。`今すぐ実行` を押した場合は、完了まで待機し、設定画面の `最終実行結果` を更新します。
...
成人向け検索で結果が 0 件だった場合も、通常検索へフォールバックして取りこぼしを減らします。
...
2. 変更をコミットして `main` に反映します
3. 同じバージョン番号で `vX.Y.Z` 形式のタグを作成します
...
`Package Extension` workflow はタグ push で起動します。`main` に push しただけでは GitHub Release は作成されません。
...
| --- | --- |
| `alarms` | 定期チェック |
| `storage` | 設定、通知済み商品 ID、新着商品一覧、未読件数、最終実行結果、Discord 再送キューの保存 |
...
- 設定、通知済み商品 ID、新着商品一覧、未読件数、最終実行結果、Discord 再送キューは端末ローカルの Extension Storage に保存します
...
- `新着商品一覧` は直近に保存された商品だけを表示します。表示件数の設定も確認してください
...
- 一時的な失敗分は次回チェック時に自動再送されます。設定画面の `最終実行結果` に失敗件数や再送状況が表示されます

### 実行中のまま止まって見える

- 実行中にブラウザを閉じた場合や PC がスリープした場合は、前回の実行が中断扱いになることがあります
- 古い `running` 状態は設定画面またはポップアップを開いた時点でエラー表示に補正されます
- その場合は `今すぐ実行` で再実行してください

### リリースが作成されない

- `main` への push だけでは Release は作成されません
- `vX.Y.Z` 形式のタグ push で `Package Extension` workflow が起動します
- Actions タブで `Package Extension` の実行結果を確認してください
